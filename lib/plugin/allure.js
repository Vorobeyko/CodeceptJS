...
module.exports = (config) => {
  ...
  if (config.executors.enable) {
    if (config.executors.gitlab) {
      const executorInfo = getGitlabExecutorInfo(config.executors.gitlab);
      createExecutorFile(config.outputDir, executorInfo);
    }
  }
...
function getGitlabExecutorInfo(param) {
  const defaultParam = {
    name: 'Gitlab',
    type: 'gitlab',
    url: '',
    buildOrder: process.env.CI_JOB_ID,
    buildName: `${process.env.CI_JOB_NAME}#${process.env.CI_JOB_ID}`,
...
function createExecutorFile(outputDir, executorInfo) {
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir);
  }
  fs.writeFile(`${outputDir}/executor.json`, JSON.stringify(executorInfo), 'utf8', (err) => {
    if (err) throw err;
  });
}
